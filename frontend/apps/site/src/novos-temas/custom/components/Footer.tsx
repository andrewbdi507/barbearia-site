    ...
    <footer style={{ backgroundColor: tokens.colors.primary, color: tokens.colors.textInverse, paddingTop: tokens.spacing.section, paddingBottom: "2rem" }}>
      <div className="max-w-[1200px] mx-auto px-6 md:px-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 pb-12 border-b" style={{ borderColor: "rgba(255,255,255,0.15)" }}>
          <div><div className="flex items-center gap-2 mb-4">{brandLogo ? <img src={brandLogo} alt={brandName} className="h-8 w-auto brightness-0 invert" /> : <Scissors className="w-5 h-5" />}<span className="text-lg font-bold">{brandName}</span></div>{description && <p className="text-xs leading-relaxed mb-4 opacity-70">{description}</p>}{socialLinks && <div className="flex gap-3">{socialLinks.map((l) => <a key={l.platform} href={l.url} target="_blank" rel="noopener noreferrer" className="w-8 h-8 flex items-center justify-center rounded-full border transition-colors" style={{ borderColor: "rgba(255,255,255,0.2)" }} aria-label={l.platform}>{si[l.platform] || <ArrowUpRight className="w-3.5 h-3.5" />}</a>)}</div>}</div>
